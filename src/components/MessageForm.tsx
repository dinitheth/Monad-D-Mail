...
export function MessageForm({ onPostMessage, posting, account }: MessageFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (message.trim() && recipient.trim()) {
      onPostMessage(message.trim(), recipient.trim());
      setMessage('');
    }
  };
      ...
      <Card className="overflow-hidden shadow-lg border-primary/20 max-w-3xl mx-auto">
        <form onSubmit={handleSubmit}>
                       ...
                       <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
                          <PopoverTrigger asChild disabled={isDisabled}>
                               <Button
                                  ...
                                  className="w-full justify-between pl-9 font-normal"
                              >
                                  {recipient ? 
                                      (contacts.find(c => c.address.toLowerCase() === recipient.toLowerCase())?.name || recipient) 
                                      : (account ? "Select recipient..." : "Connect wallet first")
                                  }
          ...
          <CardFooter className="flex justify-end p-4 py-3 bg-muted/50">
            <Button type="submit" disabled={!message.trim() || !recipient.trim() || isDisabled}>
              {posting ? (
                <>
